...
public class TreeTraversal {
    ...
    public Iterable<Node> levelOrder() {
        Queue<Node> lO = new Queue<Node>();
        Queue<Node> p = new Queue<Node>();
        lO.enqueue(root);
        while(!lO.isEmpty()) {
            Node y = lO.dequeue();
            p.enqueue(y);
            if (y.left != null) { lO.enqueue(y.left); }
            if (y.right != null) { lO.enqueue(y.right); }
        }
        return p;
    }
